...
p_st_d=0.3;
p_st_s=0.28;
regeneration_time1 = 11; 
regeneration_time2 = 11; 
...
for i=1:number_sand
    ...
    for n=1:numArrays
        ...
        for b=1:length(active_position)
           ...
           if ring{n}(5,active_position(b)) <=50
            if active_or_not_for_this_site <= p_sp_1
               ring{n}(5,active_position(b)) = ring{n}(5,active_position(b)) + 1;

               if ring{n}(2,active_position(b)) <= 80
                   ring{n}(3,active_position(b)) = ring{n}(3,active_position(b)) + regeneration_time1;
               end
               if ring{n}(2,active_position(b)) > 80
                   ring{n}(3,active_position(b)) = ring{n}(3,active_position(b)) + regeneration_time2;
               end
            end 
           end

              if ring{n}(5,active_position(b)) >50
            if active_or_not_for_this_site <= p_sp_2
               ring{n}(5,active_position(b)) = ring{n}(5,active_position(b)) + 1;
    ...
    for n=1:numArrays
        ...
        for j=1:length(position_tops)
            ...
            if ring{n}(2,position_tops(j)) <= 80
               ring{n}(3,position_tops(j)) = ring{n}(3,position_tops(j)) + regeneration_time1;
            end
            if ring{n}(2,position_tops(j)) > 80
               ring{n}(3,position_tops(j)) = ring{n}(3,position_tops(j)) + regeneration_time2;
            end
            ...
            theta_now  = gpuArray(rad2deg(theta_top_now));
               ...
               for q=2:length(I)
                   ...
                   active_or_not_here1 = rand;
                      if active_or_not_here1 <= p_st_s 
                         ring{n}(5,I(q)) = ring{n}(5,I(q)) + 1;
                         %ring{n}(3,I(q)) = ring{n}(3,I(q)) + regeneration_time;
                         if ring{n}(2,I(q)) <= 80
                            ring{n}(3,I(q)) = ring{n}(3,I(q)) + regeneration_time1;
                         end
                         if ring{n}(2,I(q)) > 80
                            ring{n}(3,I(q)) = ring{n}(3,I(q)) + regeneration_time2;
                         end
    ...
    polarscatter(sand_pile(1,exist_position),sand_pile(2,exist_position),[],c,".");
