...
for n = 1:numArrays
    ...
    ring{n} = [th_ring{n}; radius_ring{n}; zeros(1,number_sites_ring{n})];
end

number_sand = 100;
p_sp=0.05; % adding probability
p_st=0.28;
...
for i=1:number_sand
    for n=1:50
        for b=1:length(ring{n})
            active_or_not_for_this_site = rand;
            if active_or_not_for_this_site <= p_sp/n % prob change as radius change
               ring{n}(3,b) = ring{n}(3,b) + 1;        
            end
        end
    end
    for n=1:50
        [position_tops] = find(ring{n}(3,:) > 3);
