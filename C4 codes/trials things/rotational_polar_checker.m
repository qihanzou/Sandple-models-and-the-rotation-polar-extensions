...
regeneration_time_2 = 1000; % topping regerneration time
shorest_number = 5;
...
if mod(i,interval)==0 %for each time statisfy the interval,draw graph.
        figure(2)
        if i > 1;
           ss = 20*ones(1,length(his(1,:)));
           polarscatter(his(1,:),his(2,:),ss,his(3,:),'filled');
           hold on
        end
...
his = [sand_pile(1,exist_position);sand_pile(2,exist_position);sand_pile(3,exist_position)];
