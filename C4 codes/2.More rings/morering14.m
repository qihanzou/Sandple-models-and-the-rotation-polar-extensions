...
regeneration_time_1 = 40; % dropping regerneration time
regeneration_time_2 = 40; % topping regerneration time
shorest_number = 6;
add_number=2;
add_number_induce=1;
...
for i=1:number_sand
    ...
    partial_active2 = active_position(1:round(0.7*act));
    act2 = length(partial_active2);
    randomNUM2 = round(1+rand(1,1)*(act2-1));
    index_now2 = active_position(randomNUM2);
    active_or_not2 = rand;
    if active_or_not2 <= p_sp
        sand_pile(3,index_now2) = sand_pile(3,index_now2) + add_number;
        active_matrix(3,index_now2) = active_matrix(3,index_now2) + regeneration_time_1;
        life_time(3,index_now2) = life_time(3,index_now2) + lifetime1;
    end
    
    partial_active3 = active_position(1:round(0.4*act));
    act3 = length(partial_active3);
    randomNUM3 = round(1+rand(1,1)*(act3-1));
    index_now3 = active_position(randomNUM3);
    active_or_not3 = rand;
    if active_or_not3 <= p_sp
        sand_pile(3,index_now3) = sand_pile(3,index_now3) + add_number;
        active_matrix(3,index_now3) = active_matrix(3,index_now3) + regeneration_time_1;
        life_time(3,index_now3) = life_time(3,index_now3) + lifetime1;
    end
...
[exist_position1]=find(active_matrix(3,:)>0);
s1 = 10*ones(1,length(exist_position1));

if mod(i,interval)==0 
        polarscatter(sand_pile(1,exist_position),sand_pile(2,exist_position),s,sand_pile(3,exist_position),"x","black");
        hold on
        polarscatter(active_matrix(1,exist_position1),active_matrix(2,exist_position1),s1,active_matrix(3,exist_position1),"o","red");
        hold off
