...
timesteps=0;
N = [n4 n5 n6 n7 n8 n9 n10 n11 n12 n13 n14 n15 n16 n17 n18 n19 n20 n21 n22 n23 n24 n25 n26 n27 n28 n29 n30 n31 n32 n33 n34 n35 n36 n37 n38 n39 n40 41 n42 n43 n44 n45 n46 n47 n48 n49 n50];
T = [th4 th5 th6 th7 th8 th9 th10 th11 th12 th13 th14 th15 th16 th17 th18 th19 th20 th21 th22 th23 th24 th25 th26 th27 th28 th29 th30 th31 th32 th33 th34 th35 th36 th37 th38 th39 th40 th41 th42 th43 th44 th45 th46 th47 th48 th49 th50];
theta4 = [th4,zeros(1,length(th50)-length(th4))];
theta5 = [th5,zeros(1,length(th50)-length(th5))];
theta6 = [th6,zeros(1,length(th50)-length(th6))];
theta7 = [th7,zeros(1,length(th50)-length(th7))];
theta8 = [th8,zeros(1,length(th50)-length(th8))];
theta9 = [th9,zeros(1,length(th50)-length(th9))];
theta10 = [th10,zeros(1,length(th50)-length(th10))];
theta11 = [th11,zeros(1,length(th50)-length(th11))];
theta12 = [th12,zeros(1,length(th50)-length(th12))];
theta13 = [th13,zeros(1,length(th50)-length(th13))];
theta14 = [th14,zeros(1,length(th50)-length(th14))];
theta15 = [th15,zeros(1,length(th50)-length(th15))];
theta16 = [th16,zeros(1,length(th50)-length(th16))];
theta17 = [th17,zeros(1,length(th50)-length(th17))];
theta18 = [th18,zeros(1,length(th50)-length(th18))];
theta19 = [th19,zeros(1,length(th50)-length(th19))];
theta20 = [th20,zeros(1,length(th50)-length(th20))];
theta21 = [th21,zeros(1,length(th50)-length(th21))];
theta22 = [th22,zeros(1,length(th50)-length(th22))];
theta23 = [th23,zeros(1,length(th50)-length(th23))];
theta24 = [th24,zeros(1,length(th50)-length(th24))];
theta25 = [th25,zeros(1,length(th50)-length(th25))];
theta26 = [th26,zeros(1,length(th50)-length(th26))];
theta27 = [th27,zeros(1,length(th50)-length(th27))];
theta28 = [th28,zeros(1,length(th50)-length(th28))];
theta29 = [th29,zeros(1,length(th50)-length(th29))];
theta30 = [th30,zeros(1,length(th50)-length(th30))];
theta31 = [th31,zeros(1,length(th50)-length(th31))];
theta32 = [th32,zeros(1,length(th50)-length(th32))];
theta33 = [th33,zeros(1,length(th50)-length(th33))];
theta34 = [th34,zeros(1,length(th50)-length(th34))];
theta35 = [th35,zeros(1,length(th50)-length(th35))];
theta36 = [th36,zeros(1,length(th50)-length(th36))];
theta37 = [th37,zeros(1,length(th50)-length(th37))];
theta38 = [th38,zeros(1,length(th50)-length(th38))];
theta39 = [th39,zeros(1,length(th50)-length(th39))];
theta40 = [th40,zeros(1,length(th50)-length(th40))];
theta41 = [th41,zeros(1,length(th50)-length(th41))];
theta42 = [th42,zeros(1,length(th50)-length(th42))];
theta43 = [th43,zeros(1,length(th50)-length(th43))];
theta44 = [th44,zeros(1,length(th50)-length(th44))];
theta45 = [th45,zeros(1,length(th50)-length(th45))];
theta46 = [th46,zeros(1,length(th50)-length(th46))];
theta47 = [th47,zeros(1,length(th50)-length(th47))];
theta48 = [th48,zeros(1,length(th50)-length(th48))];
theta49 = [th49,zeros(1,length(th50)-length(th49))];
theta50 = th50;
%theta50 = [th4,zeros(1,length(th50)-length(th50))];
TT = [theta4;theta5;theta6;theta7;theta8;theta9;theta10;theta11;theta12;theta13;theta14;theta15;theta16;theta17;theta18;theta19;theta20;theta21;theta22;theta23;theta24;theta25;theta26;theta27;theta28;theta29;theta30;theta31;theta32;theta33;theta34;theta35;theta36;theta37;theta38;theta39;theta40;theta41;theta42;theta43;theta44;theta45;theta46;theta47;theta48;theta49;theta50];
...
for i=1:number_sand
    ...
    r_choose = randi([4 50]);
    r_coor = find(N==r_choose);
    theta_coor = TT(r_coor,:);
    %theta_coor(find(theta_coor==0))=[];

    rx = randi(length(theta_coor)) ;

    theta_choose = theta_coor(rx);

    [theta_index]=find(sand_pile(1,:) == theta_choose);
    [r_index]=find(sand_pile(2,:) == r_choose);
    index_now = intersect(theta_index,r_index);
