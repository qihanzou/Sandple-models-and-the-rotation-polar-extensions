...
for i=1:number_sand
    ...
    de = length(death_position);
    for ii=1:de
        theta_top_now  = sand_pile(1,death_position(ii));
            radius_top_now = sand_pile(2,death_position(ii));
            ...
            for q=2:z
                if ismember(I(q),active_position)==1
                    active_or_not4 = rand;
                    active_or_not5 = rand;
                    if active_or_not4 <= p_st && sand_pile(2,I(q)) ~= radius_top_now
                    ...
                    elseif active_or_not5 <= p_st_c && sand_pile(2,I(q)) == radius_top_now
                       sand_pile(3,I(q)) = sand_pile(3,I(q)) + 1;
                       active_matrix(3,I(q)) = active_matrix(3,I(q)) + regeneration_time_2;
                       life_time(3,I(q)) = life_time(3,I(q)) + lifetime2;
                    end
...
s1 = 20*ones(1,length(exist_position1));

if mod(i,interval)==0 
        polarscatter(sand_pile(1,exist_position),sand_pile(2,exist_position),s,sand_pile(3,exist_position),"pentagram");
        hold on
