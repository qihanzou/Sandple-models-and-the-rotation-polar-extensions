...
for i=1:number_sand
    ...
    if active_or_not <= p_sp
        ...
        active_matrix(3,index_now) = active_matrix(3,index_now) + regeneration_time;
    end

    [position_tops] = find(sand_pile(3,:)>0);
   
    number_top = length(position_tops);
        for j = 1: number_top
            pause(0.01)
            sand_pile(3,position_tops(j)) = 0;
            theta_top_now  = sand_pile(1,position_tops(j));
            radius_top_now = sand_pile(2,position_tops(j));
            active_matrix(3,position_tops(j)) = active_matrix(3,position_tops(j)) + regeneration_time;
            ...
            for q=2:length(I)
                if ismember(I(q),active_position)==1
                    active_or_not = rand;
                    if active_or_not <= p_st && sand_pile(2,I(q)) ~= radius_top_now
                       sand_pile(3,I(q)) = sand_pile(3,I(q)) + 1;
                       active_matrix(3,I(q)) = active_matrix(3,I(q)) + regeneration_time;
             ...
             polarscatter(sand_pile(1,exist_position),sand_pile(2,exist_position),"x"); 
