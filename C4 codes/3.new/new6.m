...
regeneration_time_1 = 150; % dropping regerneration time
regeneration_time_2 = 150; % topping regerneration time
shorest_number = 7;
add_number=2;
add_number_induce=1;
...
for i=1:number_sand
    ...
    for gg=1:length(life_position)
       ring_position = sand_pile(2,life_position(gg));
       if ring_position <=15
          life_time(3,life_position(gg)) = life_time(3,life_position(gg)) - 0.5; 
       end
       if ring_position > 15 && ring_position <=25
          life_time(3,life_position(gg)) = life_time(3,life_position(gg)) - 0.5; 
       end
       if ring_position > 25 && ring_position <=30
          life_time(3,life_position(gg)) = life_time(3,life_position(gg)) - 0.9; 
       end
       if ring_position > 30 && ring_position <=35
          life_time(3,life_position(gg)) = life_time(3,life_position(gg)) - 1.2; 
       end
       if ring_position > 35 && ring_position <=40
          life_time(3,life_position(gg)) = life_time(3,life_position(gg)) - 1.5; 
       end
       if ring_position > 40 && ring_position <=45
          life_time(3,life_position(gg)) = life_time(3,life_position(gg)) - 2; 
       end
       if ring_position > 45 && ring_position <=50
          life_time(3,life_position(gg)) = life_time(3,life_position(gg)) - 5; 
       end
    end
    ...
    if active_or_not <= p_sp
        ...
        life_time(3,index_now) = life_time(3,index_now) + lifetime1;
    end
        ...
        for j = 1: number_top
            ...
            for q=2:z
                if ismember(I(q),active_position)==1
                    ...
                    active_or_not4 = rand;
                    active_or_not5 = rand;
                    radius_top_now_yes = sand_pile(2,I(q));
                    if radius_top_now_yes > 40 && sand_pile(2,I(q)) < radius_top_now
                              sand_pile(3,I(q)) = sand_pile(3,I(q)) + add_number_induce;
                              active_matrix(3,I(q)) = active_matrix(3,I(q)) + regeneration_time_2;
                              life_time(3,I(q)) = life_time(3,I(q)) + lifetime2;
                    elseif radius_top_now_yes > 45 && sand_pile(2,I(q)) < radius_top_now
                              sand_pile(3,I(q)) = sand_pile(3,I(q)) + add_number_induce;
                              active_matrix(3,I(q)) = active_matrix(3,I(q)) + regeneration_time_2;
                              life_time(3,I(q)) = life_time(3,I(q)) + lifetime2;
                    elseif active_or_not2 <= p_st && sand_pile(2,I(q)) < radius_top_now
                           sand_pile(3,I(q)) = sand_pile(3,I(q)) + add_number_induce;
                           active_matrix(3,I(q)) = active_matrix(3,I(q)) + regeneration_time_2;
                           life_time(3,I(q)) = life_time(3,I(q)) + lifetime2;

                       if ismember(I(q)+1,active_position)==1
                          radius_top_now2 = sand_pile(2,I(q)+1);
                           if radius_top_now2 > 45 && sand_pile(2,I(q)+1) < radius_top_now
                              sand_pile(3,I(q)+1) = sand_pile(3,I(q)+1) + add_number_induce;
                              active_matrix(3,I(q)+1) = active_matrix(3,I(q)+1) + regeneration_time_2;
                              life_time(3,I(q)+1) = life_time(3,I(q)+1) + lifetime2;
                           elseif active_or_not4 <= p_st && sand_pile(2,I(q)+1) < radius_top_now
                              sand_pile(3,I(q)+1) = sand_pile(3,I(q)+1) + add_number_induce;
                              active_matrix(3,I(q)+1) = active_matrix(3,I(q)+1) + regeneration_time_2;
                              life_time(3,I(q)+1) = life_time(3,I(q)+1) + lifetime2;
                           end
                       ...
                       if ismember(I(q)-1,active_position)==1
                          radius_top_now3 = sand_pile(2,I(q)-1);
                           if radius_top_now2 > 45 && sand_pile(2,I(q)-1) < radius_top_now
                              sand_pile(3,I(q)-1) = sand_pile(3,I(q)-1) + add_number_induce;
                              active_matrix(3,I(q)-1) = active_matrix(3,I(q)-1) + regeneration_time_2;
                              life_time(3,I(q)+1) = life_time(3,I(q)+1) + lifetime2;
                           elseif active_or_not5 <= p_st && sand_pile(2,I(q)-1) < radius_top_now
                              sand_pile(3,I(q)-1) = sand_pile(3,I(q)-1) + add_number_induce;
                              active_matrix(3,I(q)-1) = active_matrix(3,I(q)-1) + regeneration_time_2;
                              life_time(3,I(q)-1) = life_time(3,I(q)-1) + lifetime2;   
                           end
...
s1 = 5*history(3,exist_position1);

if mod(i,interval)==0 
        polarscatter(sand_pile(1,exist_position),sand_pile(2,exist_position),s,sand_pile(3,exist_position),"pentagram","filled");
