...
add=2; %add how many sand in a entry
regeneration_rate=1; %the speed for regeneration
regeneration_time=4; %the time required for regeneration
move_towards_center_time = 10;

active_matrix = gpuArray(zeros(N,M));

for i=1:number_sand 
    if mod(i,2)==1
    %if i >= 0
       [x_to_c, y_to_c] = find(sand_pile >= 0);
       x_to_c=gpuArray(x_to_c);
       y_to_c=gpuArray(y_to_c);
       to_c_index = length(x_to_c);
       for n=1:to_c_index
           %add_to_center=1;
           add_to_center=sand_pile(x_to_c(n),y_to_c(n));
           if (x_to_c(n)==ceil(M/2)) && (y_to_c(n) < ceil(M/2))
              sand_pile(x_to_c(n),y_to_c(n)+1)=sand_pile(x_to_c(n),y_to_c(n)+1) + add_to_center;
              sand_pile(x_to_c(n),y_to_c(n))=0;
           end
           if (x_to_c(n)==ceil(M/2)) && (y_to_c(n) > ceil(M/2))
              sand_pile(x_to_c(n),y_to_c(n)-1)=sand_pile(x_to_c(n),y_to_c(n)-1) + add_to_center;
              sand_pile(x_to_c(n),y_to_c(n))=0;
           ...
           if (x_to_c(n) < ceil(M/2)) && (y_to_c(n) == ceil(M/2))
              sand_pile(x_to_c(n)+1,y_to_c(n))=sand_pile(x_to_c(n)+1,y_to_c(n)) + add_to_center;
              sand_pile(x_to_c(n),y_to_c(n))=0;
           end
           if (x_to_c(n) > ceil(M/2)) && (y_to_c(n) == ceil(M/2))
              sand_pile(x_to_c(n)-1,y_to_c(n))=sand_pile(x_to_c(n)-1,y_to_c(n)) + add_to_center;
           ...
           if (x_to_c(n) < ceil(M/2)) && (y_to_c(n) < ceil(M/2))
              sand_pile(x_to_c(n)+1,y_to_c(n)+1)=sand_pile(x_to_c(n)+1,y_to_c(n)+1) + add_to_center;
              sand_pile(x_to_c(n),y_to_c(n))=0;
           end
           if (x_to_c(n) > ceil(M/2)) && (y_to_c(n) < ceil(M/2))
              sand_pile(x_to_c(n)-1,y_to_c(n)+1)=sand_pile(x_to_c(n)-1,y_to_c(n)+1) + add_to_center;
    ...
    [active_x, active_y]    = find(active_matrix == 0);
    [in_active_x, in_active_y] = find(active_matrix >  0);

    in_act_index = gpuArray(length(in_active_x));
    for g=1:in_act_index
       active_matrix(in_active_x(g),in_active_y(g)) = active_matrix(in_active_x(g),in_active_y(g)) - regeneration_rate; 
    end

    act_index = gpuArray(length(active_x));
    x_choose = gpuArray(round(1+rand(1,1)*(act_index-1)));
    x_coord_now = gpuArray(active_x(x_choose));
    y_coord_now = gpuArray(active_y(x_choose));

    sand_pile(x_coord_now,y_coord_now) = sand_pile(x_coord_now,y_coord_now) + add;
    ...
    while number_top > 0
        for j = 1: number_top
            ...
            active_matrix(x_coord_tops(j),y_coord_tops(j)) = active_matrix(x_coord_tops(j),y_coord_tops(j)) + regeneration_time; 
