...
area_matrix=gpuArray(area_matrix);
% for area frequency:
A = area_matrix(find(area_matrix~=0)); 
A = gpuArray(A);
X3=[1:1:max(A)]'; 
X3=gpuArray(X3);
Y3=zeros(max(A),1); 
...
for b=1:max(A) 
C3 = numel(find(area_matrix==b)); 
C3=gpuArray(C3);
Y3(b,1)=C3;
end
% plot the log of avalanche area vs log of frequency
figure(7)
plot(log(X3),log(Y3)) % do the log sclae plot
ylabel('log of frequency')
xlabel('log of avalanche area')
title('log(avalanche area) vs log(frequency) plot')
Y3=gpuArray(Y3);
X3=gpuArray(X3);

X3(Y3==0)=[];
Y3(Y3==0)=[];
log_X3 = gpuArray(log(X3));
log_Y3 = gpuArray(log(Y3));
mdl3 = fitlm(log_X3,log_Y3);
mdl3.Coefficients
figure(8)
plot(mdl3)
ylabel('log of frequency')
...
% create video
Video = VideoWriter('middle added sandpile','MPEG-4');
open(Video);
